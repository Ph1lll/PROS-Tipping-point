...
void liftCtrl()
{
	...
	while (1)
	{
		...
		liftBtm = ((oleana.get_digital(DIGITAL_R2) && LIFTO.get() <= 115) || (oleana.get_digital(DIGITAL_R1) && LIFTO.get() >= 400)) ? 0 : 1;
...
void autonTop()
{
	while (autonGo)
	{
		lcd::set_text(3, "LBM Current: " + LBM.get_current_draw());
		lcd::set_text(4, "LFM Current: " + LFM.get_current_draw());
		lcd::set_text(5, "RBM Current: " + RBM.get_current_draw());
		lcd::set_text(6, "RFM Current: " + RFM.get_current_draw());
		// Stop the Robot
...
void disabled()
{
	if (usercontrol) usercontrol = false;
}